...
- **Jacob's first playtest read (2026-09-10, after r85):** "holy shit, this game is hard! i cant even get past stage 2." Wants to experiment with **Q23 (shot limit)**: "when there are a ton of bullets i get pushed to the bottom of the screen, but that's also where I'm weakest, so it snowballs into a death." Ideas floated: no shot limit / no limit + DDP aura / secondary shot. Fable's read: a RECOVERY problem, not a reward problem — the cap IS the point-blank invitation (Q23's r59 measurement: no cap = six checks red, aggro/passive ratio 3.15 → 1.79). Levers that keep the invitation, to measure in the referee before deciding: shot speed (frees cap slots at range; S1 MUST ≥ 1.8× close/far), higher cap + distance falloff (Q23's named alternative), and whether he bombs when pinned (death refills one bomb). Pass-cooldown says settle the stages 3–5 playtest first; also check the feeling is the pipeline and not the Bell (Q24). Peek sheets r82–r85 reviewed by Jacob: "look ok".
- **Closeout 2026-09-10 (this session):** tree has ONLY the uncommitted S5 rubric edit (Jacob's to commit) + the other session's concept folders (`docs/concepts/README.md`, `2026-09-09-soul-jazz/`, `2026-09-10-hero-sprites/`). Servers: **:8001 (python, main tree) left running on purpose — serves r85**; :8003/:8004 are the ended companion session's. `node test/shell.mjs` + `campaign-probe` PASS at r85. **Next first step:** `git add docs/CRITIC_RUBRIC.md && git commit` (Jacob), then playtest stages 3–5 via `?level=2/3/4` and the Booth, then the Q23 referee sweep if still wanted.
