...
//   2. TWO-STAGE FAKE — push a duplicate of s1 into STAGES at runtime and play
//      the expert through both: rng continuous (same objects, no reseed — and
//      stage 2 plays a DIFFERENT run than a fresh stage 1 on the same seed,
//      which is what a continuous stream looks like), lives / bombs / score
//      carried, chain reset, stage 2's timeline from stageT 0, g.level === 1,
//      the final clear is g.state === 'clear'. Then pop the fake stage.
//
...
// --- 2. two-stage fake ---------------------------------------------------------
console.log('campaign-probe — 2. two-stage fake (STAGES = [s1, s1] at runtime)');
STAGES.push(STAGES[0]);
try {
  const g = startRun(makeGame(SEED), 0, 0);
  const rng0 = g.rng, fx0 = g.fxRng, bot = makeBot(BOTS.expert);
  play(g, bot);
  console.log(`  stage 1 end → ${sig(g)}  level ${g.level}  stageT ${g.stageT}`);
  ok(g.state === 'stageclear', "non-final boss down → g.state 'stageclear'");
  ok(g.level === 0, 'g.level still 0 at the clear');
  ok(g.frame === A.frame && g.score === A.score && g.kills === A.kills, 'stage 1 of the two-stage run == the one-stage expert run (same frames/score/kills)');
  ...
  ok(g.timeline !== tl1 && g.timeline.length === tl1.length && g.timeline[0].t === 120, `stage 2 timeline rebuilt (${g.timeline.length} events, first at stageT 120)`);
  ...
  ok(g.state === 'clear' || g.state === 'gameover', `final stage ends the run: '${g.state}' (the expert enters stage 2 with the stock it has left — ${carry.lives} lives at r77's boss hp — so a game over here is the arcade contract, Pillar 4, not a plumbing fault)`);
  ok(g.level === 1, 'g.level === 1 at the end');
  ok(!(st2.frames === A.frame && st2.score === A.score && st2.kills === A.kills), 'stage 2 is a DIFFERENT run than a fresh stage 1 on the same seed (the stream continued, it was not reseeded)');
  // The final-clear path itself: same two-stage run, but from the seam the
  // ship is INVULNERABLE (the camp probes' trick — a probe-only edit of the
  // flow under test, not a rule; the r77 expert bot dies 3× on stage 1 and
  // 4× on the continued stream, so no honest bot reaches stage 2's boss until
  // the extend rule, plan §5 A, is Jacob's decision). Asserts that the LAST
  // stage's clear is today's 'clear' with today's tally.
  {
    const q = startRun(makeGame(SEED), 0, 0), b2 = makeBot(BOTS.expert);
    play(q, b2); nextStage(q); q.player.invuln = 1e9;
  ...
  const h = () => { const q = startRun(makeGame(SEED), 0, 0), b2 = makeBot(BOTS.expert); play(q, b2); if (q.state === 'stageclear') { nextStage(q); play(q, b2); } return sig(q); };
  const d1 = h(), d2 = h();
  ...
  ok(p.level === 1 && p.startLevel === 1 && p.practice === 0 && p.timeline.length === tl1.length, 'startRun(g, 0, 1): level 1, startLevel 1 (practice for the board), stage-2 timeline');
  ok(startRun(makeGame(SEED), 0, 99).level === 1, 'level clamps to STAGES.length - 1');
} finally {
  STAGES.pop();
}
ok(STAGES.length === 1, 'STAGES restored to length 1');
